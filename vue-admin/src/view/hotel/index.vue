...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            return h('div', [
              h('span', params.row.facilities.map((o) => o).join(',') || '')
            ])
          },
          search: {
            type: 'input'
  ...
  methods: {
    ...
    handleRowRemove (row, index) {
      this.$Modal.confirm({
        title: '确定删除文章吗？',
        content: `删除第${index + 1}条数据，"${row.name}"吗`,
        onOk: () => {
          deleteHotelById(row._id)
            .then((res) => {
              if (res.code === 200) {
