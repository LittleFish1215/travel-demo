...
import { getSpotCommentsAll, deleteSpotCommentById, updateSpotCommentById} from '@/api/admin'
export default {
  components: {
    Tables,
    EditModel
  ...
  methods: {
    handleDeleteBatch () {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          ...
          deleteSpotCommentById(arr).then((res) => {
    ...
    // 编辑某一行数据
    handleItemEdit (item) {
      updateSpotCommentById(item).then((res) => {
        if (res.code === 200) {
    ...
    async _getList () {
      const res = await getSpotCommentsAll({
        page: this.page - 1,
        limit: this.limit,
        options: this.option
      })
