  ...
  <div>
    <Modal v-model="showStatus" title="编辑景点属性" @on-ok="ok" @on-cancel="cancel" :loading="loading">
      <Form :model="localItem" :rules="rules" :label-width="80" ref="table">
        <FormItem label="评价人" prop="cuid.name">
          <Input v-model="localItem.cuid.name" placeholder="请输入评价人昵称"></Input>
        </FormItem>
        <FormItem label="评价内容" prop="content">
          <Input v-model="localItem.content" placeholder="请输入评价内容"></Input>
        </FormItem>
        <FormItem label="景点名称" prop="spotname">
          <Input v-model="localItem.tid.name" placeholder="请输入景点天气"></Input>
        </FormItem>
      </Form>
    </Modal>
  </div>
